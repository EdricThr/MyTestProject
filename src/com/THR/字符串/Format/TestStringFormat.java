...
public class TestStringFormat {
    ...
    private static void doFormatStringAndNum() {
        ...
        System.out.printf("%f和%<6.4f%n", 99.45);
        System.out.printf("%f和%<7f%n", 99.45);
        System.out.printf("%d和%<6d%n", 99);
    ...
    private static void doTestNumAndFormat() {
        ...
        //args[]个数和匹配转换符个数相等，程序运行正常
        System.out.println("正常args[],结果：" + str);
        str = String.format(URL_FORMAT_PATTERN, "a", "b", "c", "d");
        //若args[]个数多于匹配转换符，程序运行正常，多余的args元素不起作用
        System.out.println("多余args[],结果：" + str);

        try {
            //若args[]个数少于匹配转换符，程序报错 java.util.MissingFormatArgumentException: Format specifier 's'
            str = String.format(URL_FORMAT_PATTERN, "a", "b");
            System.out.println("args[]个数不够,结果：" + str);
        }catch (Exception e) {
            e.printStackTrace();
        }

        //若format字符串不规范，程序报错 java.util.UnknownFormatConversionException: Conversion = 'j'
        URL_FORMAT_PATTERN = "http://%j:%s/%s";
        str = String.format(URL_FORMAT_PATTERN, "a", "b", "c");
        System.out.println("format字符串格式错误,结果：" + str);
    }
}
